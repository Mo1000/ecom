<template>
  ...
  >
    <div v-for="(product, index) in store.courseList" :key="index" :class="getProductClass(index)">
      <ProductCard :product="product as CourseModel" />
    </div>
  </div>
...
const isProductsListNotEmpty = computed(() => store.courseList.length > 0)

...
import type { CreateUserModel } from '@/models/user/user.model'
...
class UserClass extends Parse.User {
  ...
  static async signUp(newUser: CreateUserModel) {
    return Parse.Cloud.run('signUp', newUser)
  }
  ...
  static async loginWithEmail(_email: string, _password: string) {
    return Parse.Cloud.run('login', { email: _email, password: _password })
  }

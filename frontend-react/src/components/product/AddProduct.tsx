...
const AddProduct: FC<Props> = (props) => {
    ...
    const [sizeList, setSizeList] = useState<string[]>(product?.sizeList ?? []);
    const [colorList, setColorList] = useState<string[]>(product?.colorList ?? []);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            let res;
            if (isUpdate) {
                res = await ProductService.updateProduct(product.objectId, data as ProductModel)
            } else {
            ...
            setProductList((prev) => [...prev, res as unknown as ProductModel])
            navigate('/')
        } catch (e) {
            console.error(e)
            const message = isUpdate ? 'Product updated failed' : 'Product created failed'
            notify(message, {
                type: "error"
            })
        } finally {
                    ...
                    <div className="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <div className="mt-2.5">
                                <input
                                    type="number"
                                    defaultValue={product?.reductionPercent}
